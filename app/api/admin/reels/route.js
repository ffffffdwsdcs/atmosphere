...
export async function POST(request) {
  try {
    ...
    const { title, sub, poster, video } = body;
    
    if (!title || !sub || !poster || !video) {
...
export async function PATCH(request) {
  try {
    const body = await request.json();
    const { id, title, sub, poster, video } = body;
    ...
    if (sub !== undefined) updateDoc.sub = String(sub).trim();
    if (poster !== undefined) updateDoc.poster = String(poster).trim();
    if (video !== undefined) updateDoc.video = String(video).trim();
