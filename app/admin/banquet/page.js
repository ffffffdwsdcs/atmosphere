...
export default function BanquetInquiriesPage() {
  ...
  const fetchBanquets = async () => {
    ...
    try {
      ...
      toast.error(err.message || 'Error loading inquiries');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`/api/admin/banquet?id=${id}`, { method: 'DELETE' });
  ...
  const filtered = items.filter((item) => {
    const matchesSearch =
      ...
      (item.event_type && String(item.event_type).toLowerCase().includes(search.toLowerCase())) ||
      (item.email && String(item.email).toLowerCase().includes(search.toLowerCase()));

    const matchesStatus = statusFilter === 'all' || item.status === statusFilter;
    const matchesDate = !dateFilter || item.event_date === dateFilter;
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Name', 'Phone', 'Email', 'Event Type', 'Event Date', 'Guests', 'Budget', 'Special Requirements', 'Status', 'Created At'];
    ...
    link.setAttribute('download', `banquet_inquiries_export_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-ivory tracking-wide">Banquet Inquiries</h1>
          <p className="text-xs text-gold/60 mt-1 uppercase tracking-wider font-sans">
            Manage inquiries for weddings, corporate gatherings and galas
          </p>
        </div>
        <button
          onClick={exportToCSV}
      ...
      <div className="bg-[#1f1209] border border-gold/15 rounded-lg shadow-xl overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-3 text-gold">
            ...
            <span className="text-[10px] uppercase tracking-widest font-sans">Syncing inquiries...</span>
          </div>
        ) : (
          <>
                  ...
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((r) => (
                      <tr key={r.id} className="hover:bg-flame/[0.04] transition-colors group">
                        ...
                        <td className="p-4 text-gold font-medium">{r.event_type}</td>
                        <td className="p-4 text-ivory/70">{r.event_date || 'N/A'}</td>
                        <td className="p-4 text-ivory/70 font-mono">{r.guest_count || 'N/A'}</td>
                        <td className="p-4 text-ivory/70 font-mono">{r.budget_range || 'N/A'}</td>
                        <td className="p-4">
                          <select
                            value={r.status}
            ...
            >
              <div>
                ...
                <div className="text-flame text-[10px] uppercase tracking-[0.4em] mb-2 font-sans font-bold">
                  Drawer · Banquet Inquiries
                </div>
                <h3 className="font-display text-2xl text-ivory">Inquiry Details</h3>
                <p className="text-[10px] text-gold/60 mt-1 uppercase tracking-widest font-mono">
                  Luxe By Atmosphere
                </p>

                <div className="mt-8 space-y-4 text-xs">
                  <DetailRow label="Client Name" value={activeItem.name} />
                  <DetailRow label="Phone" value={activeItem.phone} />
                  <DetailRow label="Email" value={activeItem.email} />
                  <DetailRow label="Event Type" value={activeItem.event_type} />
                  <DetailRow label="Event Date" value={activeItem.event_date || 'Not decided'} />
                  <DetailRow label="Guest Count" value={activeItem.guest_count || 'N/A'} />
                  <DetailRow label="Budget Range" value={activeItem.budget_range || 'N/A'} />
                  <DetailRow label="Referral Source" value={activeItem.source || 'Direct Website'} />
                  <div className="border-b border-gold/10 pb-3">
                    <span className="block text-[10px] uppercase tracking-wider text-gold/60 mb-1">Special Requirements</span>
                    <p className="text-ivory bg-[#0f0a05] p-3 rounded leading-relaxed border border-gold/5 italic max-h-32 overflow-y-auto no-scrollbar">
                      {activeItem.special_requirements || 'No special requirements detailed.'}
                    </p>
                  </div>
                  <DetailRow
                    label="Received At"
              ...
              <div className="border-t border-gold/15 pt-4 mt-6">
                ...
                <div className="flex gap-2">
                  {['new', 'contacted', 'booked', 'cancelled'].map((st) => (
                    <button
                      key={st}
                      onClick={() => handleStatusChange(activeItem.id, st)}
                      className={`flex-1 py-2 text-[9px] uppercase tracking-wider font-bold transition border cursor-pointer ${
        ...
        {confirmDeleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="font-display text-xl text-ivory">Delete Inquiry</h3>
              <p className="text-xs text-ivory/65 mt-2 leading-relaxed">
                Are you absolutely sure you want to delete this banquet inquiry? This action cannot be undone.
              </p>
