...
export default function EventBookingsPage() {
  ...
  const filtered = items.filter((item) => {
    const matchesSearch =
      ...
      (item.event_name && String(item.event_name).toLowerCase().includes(search.toLowerCase())) ||
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Name', 'Phone', 'Email', 'Event Name', 'Event Date', 'Guests', 'Special Requests', 'Status', 'Created At'];
    ...
    link.setAttribute('download', `event_bookings_export_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-ivory tracking-wide">Event Bookings</h1>
          <p className="text-xs text-gold/60 mt-1 uppercase tracking-wider font-sans">
            Manage ticket reservations for music gigs, DJ events and live dinners
          </p>
        </div>
        <button
          onClick={exportToCSV}
      ...
      <div className="bg-[#1f1209] border border-gold/15 rounded-lg shadow-xl overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-3 text-gold">
            ...
            <span className="text-[10px] uppercase tracking-widest font-sans">Syncing bookings...</span>
          </div>
        ) : (
          <>
                  ...
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((r) => (
                      <tr key={r.id} className="hover:bg-flame/[0.04] transition-colors group">
                        ...
                        <td className="p-4 text-gold font-medium">{r.event_name}</td>
                        <td className="p-4 text-ivory/70">{r.event_date || 'N/A'}</td>
                        <td className="p-4 text-ivory/70 font-mono">{r.guests || 'N/A'}</td>
                        <td className="p-4">
                          <select
                            value={r.status}
        ...
        {confirmDeleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="font-display text-xl text-ivory">Delete Booking</h3>
              <p className="text-xs text-ivory/65 mt-2 leading-relaxed">
                Are you absolutely sure you want to delete this event booking? This action cannot be undone.
              </p>
